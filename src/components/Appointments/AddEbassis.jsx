...
const AddEmbassy = () => {
  ...
  const [Embassy, setEmbassy] = useState({
    ...embassy, embassy_id: "embassy_" + Date.now() + '' + generateUniqueString(),
  ...
  const initCountries = async () => {
    ...
    if (embassy.country[0] === "loading...") {
      setTimeout(() => {
        window.location.reload();
      }, [200])
    }
    console.log(Embassy.country);
    embassy.country =  data.map((e) => e.name);
    localStorage.setItem("countries", JSON.stringify(embassy.country));
    setEmbassy({ ...Embassy });
  }
  useEffect(() => {
